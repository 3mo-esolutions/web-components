...
export class DataGridColumnsController<TData> extends Controller {
	...
	private get orderColumnWidth() {
		return !this.host.reorderabilityController.enabled ? undefined : 'max-content'
	}
	...
	private get elementExtractedColumns(): Array<DataGridColumn<TData>> {
	...
	getStickyColumnInsetInline(column: DataGridColumn<TData> | keyof typeof this.columnWidths) {
		if (typeof column !== 'object') {
			switch (column) {
				...
				case 'details':
					return `${this.columnWidths.reordering + this.columnWidths.selection}px`
				case 'actions':
					return 'auto'
			}
		}
