...
export const Multiple: StoryObj = {
	render: () => html`
		...
		<mo-file-upload-drop-area multiple .upload=${(files: Array<File>) => alert(`Upload files ${files.map(file => `"${file.name}"`).formatAsConjunction()}`)}>
			<mo-flex alignItems='center' gap='0.5rem'>
				<mo-icon icon='backup'></mo-icon>
				<span>Click OR Drag & Drop multiple files</span>
			</mo-flex>
		</mo-file-upload-drop-area>
	`
}