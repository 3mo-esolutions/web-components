...
describe('Currency', () => {
	...
	// Regression: the symbol was always resolved through a hardcoded 'de-DE', so a localization package
	// returned the German symbol no matter which language was current.
	...
	// XCG and ZWG are recent enough that not every engine's CLDR data carries a display name for them
	// yet, so they must still format via the bare code rather than throw.
